...
function App() {
  ...
  const [financeRevision, setFinanceRevision] = React.useState(0);
  ...
  const bootstrap = React.useCallback(async () => {
    ...
    try {
      ...
      setBootError(err.message || 'The app could not open its local data. Check browser storage access, then retry.');
    } finally {
      setBooting(false);
    }
  }, []);
  ...
  React.useEffect(() => {
    if (!user || USE_FIREBASE) return undefined;
    const session = Auth.getSession();
    if (!session) { setUser(null); return undefined; }
    const maxAge = Math.max(1, Number(SESSION_MAX_AGE_HOURS) || 24) * 60 * 60 * 1000;
    const remaining = Number(session.loginAt) + maxAge - Date.now();
    if (remaining <= 0) { Auth.logout(); setUser(null); return undefined; }
    const timer = setTimeout(() => { Auth.logout(); setUser(null); }, Math.min(remaining, 2147483647));
  ...
  React.useEffect(() => {
    ...
    async function onStorage(e) {
      if (!e.key || !e.key.startsWith('vcf_')) return;
      try {
        DataStore.clearCache();
        const current = await Auth.getCurrentUser();
        setUser(current);
        if (e.key === 'vcf_masterData') setMasterData(await DataStore.getMasterData());
        if (e.key === 'vcf_settings') setSettings(await DataStore.getSettings());
        if (e.key === 'vcf_transactions' || e.key === 'vcf_budgets') setFinanceRevision((value) => value + 1);
      } catch (err) {
        console.error('Could not refresh changed local data', err);
      }
    }
  ...
  const commonProps = { user, masterData, settings, reloadMasterData, reloadSettings, financeRevision };
