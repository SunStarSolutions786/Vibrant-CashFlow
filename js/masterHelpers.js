...
const categoryOptionsCache = new WeakMap();
MasterHelpers.categoryOptions = function (md) {
  if (categoryOptionsCache.has(md)) return categoryOptionsCache.get(md);
  const heads = new Map((md.heads || []).map((h) => [h.id, h]));
  const verticals = new Map((md.verticals || []).map((v) => [v.id, v]));
  const options = (md.subHeads || []).flatMap((sub) => {
    const head = heads.get(sub.headId), vertical = head && verticals.get(head.verticalId);
    if (!head || !vertical) return [];
    const type = head.appliesTo === 'both' && head.group === 'internal' ? 'internal' : head.appliesTo;
    if (!['inflow', 'outflow', 'internal'].includes(type)) return [];
    return [{ value: sub.id, label: `${vertical.name} / ${head.name} / ${sub.name} · ${type}`,
      searchText: `${vertical.name} ${head.name} ${sub.name} ${type} ${head.group || ''}`,
      selection: { type, verticalId: vertical.id, headId: head.id, subHeadId: sub.id } }];
  });
  categoryOptionsCache.set(md, options);
  return options;
};
...
MasterHelpers.validateSelection = function (md, selection, type) {
  ...
  if (!pick.verticalId || !pick.headId || !pick.subHeadId) {
    return { valid: false, error: 'Type, Vertical, Head and Sub-head are required.', snapshot: null };
  }
  const snapshot = MasterHelpers.snapshotForSelection(md, pick);
  if (!snapshot) {
    return { valid: false, error: 'The selected Vertical / Head / Sub-head hierarchy is no longer valid.', snapshot: null };
  }
  if (type) {
    ...
    if (!['inflow', 'outflow', 'both'].includes(requiredFlow) || snapshot.appliesTo !== requiredFlow) {
      ...
      return { valid: false, error: `${label} must use a ${requiredFlow} Head.`, snapshot: null };
    }
    if (type === 'internal' && snapshot.group !== 'internal') {
      return { valid: false, error: 'Internal must use a Head in the Internal group.', snapshot: null };
    }
  }
