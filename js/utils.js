...
Utils.canonicalCashRow = function (record) {
  ...
  if (!rec.date || !rec.type || !Number.isFinite(amount) || amount <= 0) return '';
  return JSON.stringify([
    'cash-v5', rec.cashVerticalId || '', rec.date, rec.type, Utils.netCash(rec) < 0 ? 'out' : 'in', amount, text(rec.particulars || 'Cash entry'),
  ]);
};

Utils.cashImportHash = async function (canonical, occurrence) {
  if (!canonical) return '';
  return 'cash-v5:' + await Utils.hashText(`${canonical}|occurrence:${occurrence}`);
};

// A balance is only called latest when statement ordering identifies it.
// New uploads explicitly record order; legacy batches with varying balances
// on the same date are shown as uncertain rather than guessing from row index.
Utils.latestStatementBalance = function (records) {
  const rows = records.filter((r) => r.status !== 'void');
  if (!rows.length) return null;
  const newest = rows.slice().sort((a, b) => String(b.importedAt || b.createdAt || '').localeCompare(String(a.importedAt || a.createdAt || '')))[0];
  const batch = rows.filter((r) => (r.importBatchId || r.id) === (newest.importBatchId || newest.id));
  const ordered = batch.every((r) => r.statementOrder === 'asc' || r.statementOrder === 'desc');
  if (!ordered && new Set(batch.map((r) => Number(r.closingBalance))).size > 1) return { ...newest, balanceUncertain: true };
  const direction = newest.statementOrder === 'desc' ? 1 : -1;
  const chosen = batch.slice().sort((a, b) => direction * ((Number(a.statementRowOrder) || 0) - (Number(b.statementRowOrder) || 0)))[0];
  // Overlapping files may have been imported in any order. A differing
  // closing balance from another batch is not evidence of chronology.
  const otherBatches = rows.filter((r) => (r.importBatchId || r.id) !== (newest.importBatchId || newest.id));
  if (otherBatches.some((r) => Number(r.closingBalance) !== Number(chosen.closingBalance))) return { ...chosen, balanceUncertain: true };
  return chosen;
};
Utils.balanceCandidatesKey = async function (records) {
  return Utils.hashText(JSON.stringify(records.map((r) => [r.id, r.closingBalance, r.statementOrder || '', r.statementRowOrder || 0]).sort((a, b) => a[0].localeCompare(b[0]))));
};
Utils.budgetRevisionKey = function (records) {
  return JSON.stringify(records.map((r) => [r.id, Number(r._revision) || 0]).sort((a, b) => a[0].localeCompare(b[0])));
};
...
// ---- Server timestamps as storable data -------------------------------------------
// Firestore Timestamps (or Dates) -> { seconds, nanoseconds } and back, exactly.
Utils.stampOf = function (value) {
  if (!value) return { seconds: 0, nanoseconds: 0 };
  if (typeof value.seconds === 'number') return { seconds: value.seconds, nanoseconds: Number(value.nanoseconds) || 0 };
  const ms = value instanceof Date ? value.getTime() : Number(value) || 0;
  return { seconds: Math.floor(ms / 1000), nanoseconds: (((ms % 1000) + 1000) % 1000) * 1e6 };
};
Utils.timestampFromStamp = function (fs, stamp) {
  const seconds = Number(stamp && stamp.seconds) || 0, nanoseconds = Number(stamp && stamp.nanoseconds) || 0;
  return fs.Timestamp ? new fs.Timestamp(seconds, nanoseconds) : new Date(seconds * 1000 + Math.floor(nanoseconds / 1e6));
};

// ---- SHA-256 fingerprints for import de-duplication ----------------------------
