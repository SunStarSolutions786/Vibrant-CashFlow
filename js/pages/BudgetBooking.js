...
function BudgetBookingPage({ user, masterData }) {
  ...
  useUnsavedChanges(Object.keys(dirty).length > 0 || replaceMonth || saving || importBusy);

  const load = React.useCallback(async () => {
    setLoading(true);
    setLoadError('');
    try {
      const all = await DataStore.getBudgets(Utils.addMonths(month, -1), month);
      setBudgets(all);
      const map = {};
  ...
  const groupedBudgets = Utils.groupBy(budgets.filter((item) => item.month === month), (item) => item.subHeadId);
  const duplicateBudgets = Object.entries(groupedBudgets).filter(([, rows]) => rows.length > 1);
  async function reconcile(subHeadId, keep) {
    if (saving || importBusy) return;
    if (!window.confirm(`Keep Budget ${Utils.formatCurrency(keep.budget)} / Booking ${Utils.formatCurrency(keep.booking)} for this category? Other duplicate rows will be archived and removed from totals.`)) return;
    setSaving(true);
    try {
      await DataStore.reconcileBudgetDuplicates(month, subHeadId, keep.id, Utils.budgetRevisionKey(groupedBudgets[subHeadId]));
      await load(); Toast.success('Duplicate budgets reconciled.');
  ...
  return (
    <div>
      {duplicateBudgets.length > 0 && <SectionCard title="Duplicate budgets need review">
        <p className="error-text">Older saves created multiple budgets for the same category. Review each set and keep the correct figures before editing this month.</p>
        {duplicateBudgets.map(([subHeadId, rows]) => <div key={subHeadId} style={{ marginBottom: 16 }}>
          <b>{MasterHelpers.resolveTransactionChain(masterData, rows[0]).subHeadName || subHeadId}</b>
          {rows.map((r) => <div key={r.id} className="toolbar" style={{ marginTop: 8 }}><span>Budget {Utils.formatCurrency(r.budget)} · Booking {Utils.formatCurrency(r.booking)}</span>
            <button className="btn btn-secondary btn-sm" disabled={saving || importBusy || Object.keys(dirty).length > 0} onClick={() => reconcile(subHeadId, r)}>Keep these figures</button></div>)}
        </div>)}
      </SectionCard>}
      <div className="toolbar">
        <button type="button" className="btn btn-secondary" disabled={saving || importBusy} onClick={() => { if ((!Object.keys(dirty).length && !replaceMonth) || window.confirm('Discard the current budget draft and reload?')) load(); }}>Reload</button>
        <label className="visually-hidden" htmlFor="budget-month">Budget month</label>
        <input id="budget-month" className="input" type="month" disabled={saving || importBusy} style={{ maxWidth: 180 }} value={month} onChange={(e) => requestMonthChange(e.target.value)} />
        ...
        <label className={Utils.classNames('btn btn-secondary', importBusy && 'disabled')} style={{ cursor: importBusy ? 'not-allowed' : 'pointer' }} tabIndex={importBusy ? -1 : 0}
          ...
          <input ref={fileRef} className="visually-hidden" tabIndex="-1" type="file" accept=".xlsx,.xls" onChange={onImportFile} disabled={importBusy || saving} />
        </label>
              ...
              <tbody>
                {g.heads.map((h) => h.subHeads.map((s, i) => (
                  <tr key={s.id}>
                    ...
                    <td className="cell-num"><input aria-label={`${s.name} budget`} className="input" type="number" min="0" step="0.01" disabled={saving || importBusy} style={{ textAlign: 'right' }} value={draft[s.id] && draft[s.id].budget != null ? draft[s.id].budget : ''} onChange={(e) => setCell(s.id, 'budget', e.target.value)} placeholder="0" /></td>
                    <td className="cell-num"><input aria-label={`${s.name} booking`} className="input" type="number" min="0" step="0.01" disabled={saving || importBusy} style={{ textAlign: 'right' }} value={draft[s.id] && draft[s.id].booking != null ? draft[s.id].booking : ''} onChange={(e) => setCell(s.id, 'booking', e.target.value)} placeholder="0" /></td>
                  </tr>
                )))}
              </tbody>
            </table>
