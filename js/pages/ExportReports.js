...
function buildPivotSheet(wb, sheetName, vGroups, txns, valueFn, months, masterData) {
  ...
  for (const transaction of txns) {
    const key = MasterHelpers.snapshotKey(MasterHelpers.resolveTransactionChain(masterData, transaction));
    if (!amountsByDay.has(transaction.date)) amountsByDay.set(transaction.date, new Map());
    const daily = amountsByDay.get(transaction.date);
    daily.set(key, (daily.get(key) || 0) + valueFn(transaction));
  }
  ...
  months.forEach((m) => {
    ...
    for (let d = 1; d <= dCount; d++) {
      ...
      const dayAmounts = amountsByDay.get(iso);
      ...
      plan.forEach((p, i) => {
        ...
        let value;
        if (p.type === 'sub') {
          value = dayAmounts?.get(p.key) || 0;
          subValues[p.key] = value;
        } else {
          value = Utils.sumBy(p.groupSubKeys, (key) => subValues[key] || 0);
          rowGrand += value;
        }
        ws.getCell(r, cellCol).value = value; ws.getCell(r, cellCol).numFmt = '#,##0.00';
        if (p.type === 'total') { ws.getCell(r, cellCol).font = { bold: true }; ws.getCell(r, cellCol).fill = fillArgb(tint(p.color, 195)); }
        colTotals[i] += value;
      });
      ws.getCell(r, grandTotalCol).value = rowGrand; ws.getCell(r, grandTotalCol).numFmt = '#,##0.00'; ws.getCell(r, grandTotalCol).font = { bold: true };
      monthGrand += rowGrand;
      r++;
    }

    ws.getCell(r, 2).value = `${Utils.monthLabel(m)} — Monthly Total`;
    plan.forEach((p, i) => { const cell = ws.getCell(r, LEAD_COLS + 1 + i); cell.value = colTotals[i] || 0; cell.numFmt = '#,##0.00'; });
    ws.getCell(r, grandTotalCol).value = monthGrand; ws.getCell(r, grandTotalCol).numFmt = '#,##0.00';
...
async function buildOutflowSheet(wb, masterData, txns, budgets, months) {
  ...
  // Resolve every record's category once and bucket it by category + month.
  // Scanning all records for every cell made large ranges freeze the browser.
  const monthSet = new Set(months);
  const cellKey = (categoryKey, month) => `${month}\u0000${categoryKey}`;
  const plannedByCell = new Map();
  budgets.forEach((b) => {
    if (!monthSet.has(b.month)) return;
    const k = cellKey(MasterHelpers.snapshotKey(MasterHelpers.resolveTransactionChain(masterData, b)), b.month);
    const current = plannedByCell.get(k) || { budget: 0, booking: 0 };
    current.budget += Number(b.budget) || 0;
    current.booking += Number(b.booking) || 0;
    plannedByCell.set(k, current);
  });
  const paidByCell = new Map();
  relevantTxns.forEach((t) => {
    const month = Utils.monthKey(t.date);
    if (!monthSet.has(month)) return;
    const k = cellKey(MasterHelpers.snapshotKey(MasterHelpers.resolveTransactionChain(masterData, t)), month);
    paidByCell.set(k, (paidByCell.get(k) || 0) + (Number(Utils.netOutflow(t)) || 0));
  });
  ...
  categories.forEach(({ key, chain }) => {
    ...
    months.forEach((m, mi) => {
      const planned = plannedByCell.get(cellKey(key, m)) || { budget: 0, booking: 0 };
      const budget = planned.budget;
      const booking = planned.booking;
      const paid = paidByCell.get(cellKey(key, m)) || 0;
      ...
      [startCol, startCol + 1, startCol + 2].forEach((c) => { ws.getCell(r, c).numFmt = '#,##0.00'; });
    });
    r++;
  });
...
async function buildRawDataSheet(wb, masterData, txns, months) {
  ...
  const monthSet = new Set(months);
  const rows = txns.filter((t) => monthSet.has(Utils.monthKey(t.date))).sort((a, b) => a.date.localeCompare(b.date));
  // Bulk row insertion is several times faster than per-cell writes on large exports.
  ws.addRows(rows.map((t) => {
  ...
  ws.getColumn(13).numFmt = '#,##0.00'; ws.getColumn(14).numFmt = '#,##0.00';
...
function ExportReportsPage({ masterData }) {
  ...
  async function loadReportRows(months) {
    ...
    // Reports always use current server data; the fresh read also refreshes the report cache.
    const report = await DataStore.getReportData({ fromMonth: first, toMonth: last, budgets: true, fresh: true });
    return { transactions: report.transactions, budgets: report.budgets };
  }
  ...
  async function generate() {
    ...
    try {
      ...
      await SpreadsheetLibraries.load('ExcelJS');
  ...
  async function generateHtml() {
    ...
    try {
      ...
      console.error(err);
      Toast.error(err.message || 'Could not generate the analysis report.');
    } finally {
      setBusy(false);
    }
  }
