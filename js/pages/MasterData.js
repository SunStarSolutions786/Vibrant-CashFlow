...
function MasterDataPage({ masterData, reloadMasterData }) {
  ...
  const subsByHead = Utils.groupBy(md.subHeads, (sub) => sub.headId);
  const classifyGroups = md.heads
    .map((h) => {
      const vertical = MasterHelpers.verticalName(md, h.verticalId);
      const allSubs = subsByHead[h.id] || [];
  ...
  const classifyRows = classifyGroups.flatMap((g) => (g.subs.length ? g.subs : [null]).map((sub) => ({ ...g, subs: sub ? [sub] : [] })));
  const totalRows = tab === 'banks' ? banksFiltered.length : tab === 'verticals' ? verticalsFiltered.length : classifyRows.length;
  const currentPage = Math.min(page, Math.max(0, Math.ceil(totalRows / 50) - 1));
  const visibleRows = (rows) => rows.slice(currentPage * 50, currentPage * 50 + 50);
    ...
    <div>
      ...
      {tab === 'verticals' && (
        <div className="table-wrap"><table className="dtable">
          ...
          <tbody>{visibleRows(verticalsFiltered).map((v) => (
      ...
      {tab === 'banks' && (
        <div className="table-wrap"><table className="dtable">
          ...
          <tbody>{visibleRows(banksFiltered).map((b) => (
      ...
      {tab === 'classify' && (
        <div className="table-wrap"><table className="dtable">
          ...
          <tbody>
            ...
            {visibleRows(classifyRows).map((g) => {
