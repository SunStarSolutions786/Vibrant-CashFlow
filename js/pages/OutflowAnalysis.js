...
function OutflowAnalysisPage({ masterData }) {
  ...
  React.useEffect(() => {
    let alive = true;
    const fresh = refreshNext.current;
    refreshNext.current = false;
    setLoading(true);
    setLoadError('');
    (async () => {
      try {
        const fromMonth = mode === 'month' ? month : Utils.addMonths(month, -5);
        const report = await DataStore.getReportData({ fromMonth, toMonth: month, budgets: true, fresh });
        if (alive) { setTxns(report.transactions); setBudgets(report.budgets); setAvailableMonths(report.months); setLoadedAt(report.loadedAt); }
      } catch (err) {
        console.error(err);
        if (alive) {
  ...
  return (
    <div>
      <div className="toolbar">
        ...
        <DataFreshness loadedAt={loadedAt} onRefresh={() => { refreshNext.current = true; setRefreshKey((n) => n + 1); }} />
      </div>
