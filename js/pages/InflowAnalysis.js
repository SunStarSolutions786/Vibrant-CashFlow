...
function InflowAnalysisPage({ masterData }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const report = await DataStore.getReportData({ fromMonth: month, toMonth: month, fresh });
        if (alive) { setTxns(report.transactions); setAvailableMonths(report.months); setLoadedAt(report.loadedAt); }
      } catch (err) {
        console.error(err);
        if (alive) {
