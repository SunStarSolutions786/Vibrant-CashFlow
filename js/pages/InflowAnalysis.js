...
function buildMonthOptions(txns, budgets, includeCurrent) {
  const set = new Set((txns || []).map((t) => Utils.monthKey(t.date)).filter(Boolean));
  (budgets || []).forEach((b) => { if (b.month) set.add(b.month); });
  if (includeCurrent) set.add(Utils.monthKey(Utils.todayISO()));
  return Array.from(set).sort().reverse().map((m) => ({ value: m, label: Utils.monthLabel(m) }));
}
...
function InflowAnalysisPage({ masterData, financeRevision }) {
    ...
    (async () => {
      try {
        const [list, months] = await Promise.all([
          DataStore.getTransactions({ month, all: true }),
        ...
        console.error(err);
        if (alive) {
          setLoadError('Could not load inflow analysis.');
          Toast.error('Could not load inflow analysis.');
        }
      } finally {
