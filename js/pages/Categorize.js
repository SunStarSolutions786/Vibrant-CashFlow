    ...
    try {
      const [all, rawMemory] = await Promise.all([
        DataStore.getTransactions({
          status: tab === 'pending' ? TXN_STATUS.UNCATEGORIZED : '',
          ordered: tab !== 'pending',
          limit: FIREBASE_QUERY_PAGE_SIZE,
        }),
      ...
      setDrafts(() => {
        ...
        all.forEach((t) => {
          ...
          if (t.status === TXN_STATUS.CATEGORIZED) {
            ...
            const key = Utils.mappingMemoryKey(t.remarksBank || t.particulars);
      ...
      console.error(err);
      Toast.error(DataStore.describeError(err, 'Could not load transactions for categorization.'));
    } finally {
      setLoading(false);
    }
  }, [masterData, tab]);
