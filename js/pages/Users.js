...
function UsersPage({ user, settings, reloadSettings }) {
  ...
  const load = React.useCallback(async () => {
    ...
    catch (err) { console.error(err); Toast.error('Could not load users.'); }
  ...
  async function updateLockDays(val) {
    ...
    try {
      ...
      console.error(err); Toast.error('Could not update the setting.'); return false;
    }
  }
  ...
  async function toggleActive(u) {
    ...
    try {
      ...
      await load();
    } catch (err) { console.error(err); Toast.error('Could not update that user.'); }
  }
  ...
  async function removeUser(id) {
    ...
    try {
      ...
      await load();
    } catch (err) { console.error(err); Toast.error('Could not delete that user.'); }
  }
  ...
  return (
    <div>
            ...
            {users.map((u) => (
              <tr key={u.id}>
                ...
                <td style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                  ...
                  <button className="btn btn-ghost btn-sm" onClick={() => setModal({ resetPw: u })}>Reset Password</button>
                  ...
                  <button className="btn btn-ghost btn-sm" onClick={() => setConfirmDelete(u)}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      {confirmDelete && (
        <ConfirmModal title="Delete user" tone="danger" confirmLabel="Delete"
          message={`Delete "${confirmDelete.name}" (@${confirmDelete.username})? This cannot be undone.`}
...
function UserFormModal({ mode, existing, users, currentUserId, onClose, onDone }) {
  ...
  async function submit(e) {
    ...
    if (mode === 'resetPw') {
      if (USE_FIREBASE) {
        ...
        try {
          ...
          onDone();
        } catch (err) { console.error(err); Toast.error('Could not send the password reset email.'); }
        finally { setBusy(false); }
        return;
      }
      ...
      setBusy(true);
      try {
        await DataStore.upsertUser({ ...existing, passwordHash: await Utils.hashText(password) });
        Toast.success('Password reset.');
        onDone();
      } catch (err) { console.error(err); Toast.error('Could not reset the password.'); }
      finally { setBusy(false); }
      return;
    }
    if (!name.trim() || !username.trim()) { Toast.error(`Name and ${USE_FIREBASE ? 'email' : 'username'} are required.`); return; }
    if (USE_FIREBASE && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(username.trim())) { Toast.error('Enter a valid email address.'); return; }
    ...
    try {
      if (mode === 'add') {
        const id = USE_FIREBASE
          ? await Auth.createFirebaseUser(username.trim(), password)
          : Utils.genId('usr');
        await DataStore.upsertUser({
          id, name: name.trim(), username: username.trim(), role, active: true,
          ...(USE_FIREBASE ? {} : { passwordHash: await Utils.hashText(password) }), createdAt: Utils.todayISO(),
        });
      } else {
      ...
      Toast.success('Saved.');
      onDone();
    } catch (err) { console.error(err); Toast.error('Could not save that user.'); }
    finally { setBusy(false); }
  }

  const title = mode === 'add' ? 'Add User' : mode === 'edit' ? 'Edit User' : `Reset Password — ${existing.name}`;
      ...
      <form onSubmit={submit}>
        {mode !== 'resetPw' && (
          <React.Fragment>
            ...
            <div className="field"><label>{USE_FIREBASE ? 'Email' : 'Username'}</label><input aria-label={USE_FIREBASE ? 'Email' : 'Username'} type={USE_FIREBASE ? 'email' : 'text'} className="input" required autoComplete="username" value={username} onChange={(e) => setUsername(e.target.value)} /></div>
        ...
        {(mode === 'add' || (mode === 'resetPw' && !USE_FIREBASE)) && (
          <div className="field"><label>{mode === 'add' ? 'Password' : 'New Password'}</label><input aria-label={mode === 'add' ? 'Password' : 'New Password'} className="input" required minLength="8" type="password" autoComplete="new-password" value={password} onChange={(e) => setPassword(e.target.value)} /></div>
        )}
        {mode === 'resetPw' && USE_FIREBASE && <p className="help-text">A secure Firebase password-reset link will be emailed to {existing.username}.</p>}
      </form>
    </Modal>
  );
}
