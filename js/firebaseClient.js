...
try {
    ...
    async function listAll(collectionName, options = {}) {
      ...
      if (!orders.some((item) => item.field === '__name__')) {
        const finalDirection = orders.length ? orders[orders.length - 1].direction : 'asc';
        orders.push({ field: '__name__', direction: finalDirection });
      }
      const all = [];
      let after = null;
    ...
    async function saveRevisioned(collectionName, id, value, expectedRevision, conflictMessage) {
      ...
      return fs.runTransaction(db, async (transaction) => {
        ...
        if (Number(expectedRevision) !== currentRevision) {
          throw new Error(conflictMessage || 'This record changed elsewhere. Reload and try again.');
        }
        const record = clean({ ...value, _revision: currentRevision + 1 });
        delete record.id;
    ...
    window.FirebaseBridge = {
      list, listAll, getOne, setOne, saveRevisioned, putMany, deleteOne, deleteMany,
